...
namespace BANHANG
{
    public partial class frmSendType : Form
    {
        DataTable data;
        SendTypeOB ob;
        ...
        private void frmSendType_Load(object sender, EventArgs e)
        {
        ...
        private void btnCapnhat_Click(object sender, EventArgs e)
        {
            if (!btnCapnhat.Enabled)
                return;
            if (txtSendType_Name.Text == string.Empty)
            {
                MessageBox.Show("Tên không được rỗng!", "Thông báo");
                txtSendType_Name.Focus();
                return;
            }
            if (!UpdateData())
                return;
        ...
        public void LoadData()
        {
            ...
            data = SendTypeCtr.Seach(objKeywords);
        ...
        {
            ob = new SendTypeOB();
            txtSendType_Name.Clear();
            txtSendType_Description.Clear();
            chIsActive.Checked = true;
            txtSendType_ID.Clear();
        }
        private bool EditData()
        {
            ...
            {
                ob = new SendTypeOB();
                ob = SendTypeCtr.Select(int.Parse(grvDanhsach.CurrentRow.Cells["colSendType_ID"].Value.ToString()));
                txtSendType_ID.Text = ob.SendType_ID.ToString();
                txtSendType_Name.Text = ob.SendType_Name;
                txtSendType_Description.Text = ob.SendType_Description;
        ...
        private bool UpdateData()
        {
            ...
            {
                ob = new SendTypeOB();
                ob.SendType_Name = txtSendType_Name.Text.Trim();
                ob.SendType_Description = txtSendType_Description.Text.Trim();
                ob.IsActive = chIsActive.Checked;
               
                if (FormState == FormStateType.EDIT)
                {
                    ob.SendType_ID = int.Parse(txtSendType_ID.Text.Trim());
                    ob.ModifiedBy = frmMain.obUser.User_ID;
                    SendTypeCtr.Update(ob);
                }
                else
                {
                    ob.CreatedBy = frmMain.obUser.User_ID;
                    SendTypeCtr.Insert(ob);
                }
                kq = true;
            }
            catch
        ...
        private bool DeleteData()
        {
            ...
            {
                ob=new SendTypeOB();
                ob.SendType_ID = int.Parse(grvDanhsach.CurrentRow.Cells["colSendType_ID"].Value.ToString());
                ob.ModifiedBy = frmMain.obUser.User_ID;
                SendTypeCtr.Delete(ob);
                kq = true;
            }
            catch
            {
        ...
        private void btnOrderIndex_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                frm.InitOrderList(data, "SendType_ID", "SendType_Name");
                frm.ShowDialog();
                lstOrderIndex = frm.ValueMemberList;
                if (lstOrderIndex.Count > 0)
                {
                    SendTypeCtr.Update_Index(lstOrderIndex.ToList());
